...
3. **オプショナル機能の拡張関係 (`UC200 <.. UC201`, `UC200 <.. UC202`)**  
   スタート後のヒント特定（`UC-201`）およびそれに基づく経路計画（`UC-202`）は、計画走行を行う場合のみに自動で追加されるオプショナルなフローであるため、拡張関係 (`<<extend>>`) としてベースとなる `UC-200` に矢印を向けています。
